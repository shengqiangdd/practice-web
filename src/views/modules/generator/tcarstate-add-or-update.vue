    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/tcarstate/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.carid = data.tCarState.carid
                this.dataForm.begintime = data.tCarState.begintime
                this.dataForm.endtime = data.tCarState.endtime
                this.dataForm.status = data.tCarState.status
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/tcarstate/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
